...
async def upload_file(room_id: int, user_id: str, upload: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    new_file = FileModel(
        ...
        data=file_data,
        user_id=user_id,     # <-- keep UUID string
        room_id=room_id,
    )
    db.add(new_file)
    db.commit()
...
def download_file(file_id: int, db: Session = Depends(get_db)):
    ...
    return StreamingResponse(io.BytesIO(file.data), media_type=file.content_type, headers={"Content-Disposition": f"attachment; filename={file.filename}"})
