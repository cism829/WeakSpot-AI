...
import { listMyNotes, createNote, deleteNote, semanticSearchNotes } from "../lib/api";
...
export default function Notes() {
  ...
  async function handleSearch(e) {
    ...
    try {
      ...
      setSearchErr(e?.detail || e?.message || "Search failed.");
      setSearchResults([]);
    } finally {
      setSearchBusy(false);
    }
  ...
  return (
    <div className="container">
      ...
      <div className="grid grid--2">
        ...
        <Card title="Search your notes" full>
          <form
          ...
          >
            <input
              type="text"
              className="input"
              placeholder="Search concepts, phrases, etc."
              value={searchQuery}
              onChange={(e) => {
          ...
          {searchResults.length > 0 && (
            <ul className="list">
              {searchResults.map((r) => {
                const noteMeta = notes.find((n) => n.id === r.note_id);
                const label =
                  noteMeta?.filename ||
                  noteMeta?.preview_text ||
                  `(note ${r.note_id.slice(0, 8)}…)`;
                  ...
                  >
                    <div className="col">
                      <div className="muted" style={{ fontSize: 12 }}>
                        {label} · chunk #{r.chunk_index} · dist{" "}
                    ...
                    {noteMeta && (
                      <button
                        ...
                        type="button"
                        onClick={() => nav(`/notes-analysis/${noteMeta.id}`)}
                      >
                        Open
                      </button>
                    )}
          ...
          {!searchBusy &&
            searchResults.length === 0 &&
            searchQuery.trim() && (
              <div className="muted">No matches for this query.</div>
            )}
        </Card>

        <Card title="Your notes" full>
          <ul className="list">
            {notes.map((n) => (
              <li key={n.id} className="list__row">
                <div className="col">
                  ...
                  <div className="ellipsis">{n.filename || <i>(no text yet)</i>}</div>
                </div>
